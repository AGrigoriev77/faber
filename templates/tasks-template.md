...
**Tests**: The examples below include test tasks. Tests are MANDATORY. Every user story MUST have [TEST] tasks that are written and fail BEFORE implementation tasks can begin (TDD Gate).
...
<!-- 
  ...
  The /faber.tasks command MUST replace these with actual tasks based on:
...
### Tests for User Story 1 (MANDATORY — TDD Gate: must FAIL before implementation) 🔴
...
### Tests for User Story 2 (MANDATORY — TDD Gate: must FAIL before implementation) 🔴
...
### Tests for User Story 3 (MANDATORY — TDD Gate: must FAIL before implementation) 🔴
...
- [TEST] tasks MUST be written and FAIL before [IMPL] tasks begin (TDD Gate)
