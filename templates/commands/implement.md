...
7. **TDD Gate Check** (MANDATORY before any [IMPL] task):
   ...
   - Run the test suite — all tests must **FAIL** (red phase confirms tests are real, not no-ops)
   - **If any [TEST] task is incomplete**: STOP and list missing tests. Do not proceed to implementation.
...
Note: This command assumes a complete task breakdown exists in tasks.md. If tasks are incomplete or missing, suggest running `/faber.tasks` first to regenerate the task list.
